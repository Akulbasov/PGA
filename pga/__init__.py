...
class Product(object):
    ...
    # Class for manipulation with products attributes (CV3,Google Analytics)
    ...
    def __init__(self):
        ...
        self.__set_settings_products(**{'analytics':'v3'})
        
    def __set_settings_products(self,**settings_products):
        ...
        for key in self.new_settings_product:
            ...
            Product.__select_products(self,0)
            self.__choose_output()
        return self
        
    def __del_products(self,index):
        self.__settings_products = self.__settings_products[self.__settings_products.index.values!=index]
        return self
        
    def __select_products(self,index):
    ...
    def __choose_output(self,format='DataFrame'):
    ...
    def __get_select_products(self):
...
class Request(Batch):
    ...
    def __init__(self,facet_chunk,count_day_slice,pagesize=ps,pagetoken=pt):
...
class PGA(Product,PandasEvents,Connection,Request,ExtraAppsMetaCdm,ExtraAppsManagementApi):
    ...
    def __init__(self,key_file_location=None,type_of_connection=None,facet_chunk=fc,count_day_slice=cds):
        


        '''
        
        На текущий момент основная идея представить эти три блока в формате Pandas, 
        некоторая админка: Settings (Request,Authorization,Any Product library)
    ...
    def get_dataframe(self,groupby=True):
